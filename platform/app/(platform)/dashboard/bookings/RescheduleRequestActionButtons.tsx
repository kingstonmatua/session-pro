...
export function RescheduleRequestActionButtons({ requestId }: Props) {
  ...
  async function handleAccept() {
    ...
    setError('');
    try {
      const res = await fetch(`/api/reschedule-requests/${requestId}/accept`, { method: 'POST' });
      if (!res.ok) {
  ...
  async function handleDecline() {
    ...
    setError('');
    try {
      const res = await fetch(`/api/reschedule-requests/${requestId}/decline`, { method: 'POST' });
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
